...
const UnifiedSearchForm = ({ activeTab, onClose }) => {
  ...
  const [fromLocation, setFromLocation] = useState(fetchedFromLocation);
  const [toLocation, setToLocation] = useState(fetchedToLocation);
  ...
  const handleLocationSelect = (cityData) => {
    if (currentField === "from") {
      ...
      setFromLocationToStore(cityData);
    } else if (currentField === "to") {
      setToLocation(cityData);
      setToLocationToStore(cityData);
    }
  ...
  useEffect(() => {
    setFromLocation(fetchedFromLocation);
  }, [fetchedFromLocation]);
